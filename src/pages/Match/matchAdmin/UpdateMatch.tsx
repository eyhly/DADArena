...
import { useCreateNote, useCreateRound, useUpdateMatch } from "../../../services/mutation";
...
const UpdateMatch: React.FC<UpdateMatchModalProps> = ({
  ...
  const noteRedSubmit: SubmitHandler<Notes> = (data) => {
      createNotes.mutate(
        ...
        {
          onSuccess: () => {
            Toast.fire({ icon: 'success', text: 'Notes added successfully!' });
            queryClient.invalidateQueries({ queryKey: ['notes', eventId] });
          },
          onError: (error) => {
            Toast.fire({ icon: 'error', text: error instanceof Error ? error.message : 'An unexpected error occurred' });
          },
        }
      );
  };
  ...
  return (
      <Dialog open={open} onClose={handleClose} sx={{zIndex:1400}}>
        ...
        <DialogContent>
          ...
          <Box component='form' onSubmit={handleSubmit(onSubmit)}>
            ...
            <Button
              variant="contained"
              onClick={() => appendRound({matchRound: length + 1, teamRedScore: 0, teamBlueScore: 0 })}
              startIcon={<Add />}
              fullWidth
              sx={{ mb: 2 }}
            >
            ...
            {noteFields.map((field, index) => (
              <Grid container spacing={2} key={field.id} mb={2}>
                <Grid item xs={10}>
                  <Controller
                    ...
                    render={({ field }) => (
                      <TextField
                        {...field}
                        label={`Note for ${field.teamId === teamRedId ? 'Team Red' : 'Team Blue'}`}
                        variant="outlined"
                        multiline
                        rows={3}
                        fullWidth
                ...
                <Grid item xs={2}>
                  <IconButton onClick={() => removeNote(index)}><Remove /></IconButton>
                  <IconButton
                    onClick={() => {
                      const noteData = {
                        eventId: eventId!,
                      ...
                      handleSubmit(() => noteSubmit([noteData]))(); 
                    }}
                  >
                    <Check />
                  </IconButton>
            ...
            <Button variant="contained" onClick={() => appendNote({ teamId: teamBlueId, description: "" })} startIcon={<Add />} fullWidth sx={{ mb: 2 }}>Add Note for Team Blue</Button>
            <Button variant="contained" onClick={() => appendNote({ teamId: teamRedId, description: "" })} startIcon={<Add />} fullWidth sx={{ mb: 2 }}>Add Note for Team Red</Button>
